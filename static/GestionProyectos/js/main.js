...
var ajax=(function($){
      ...
      $("#AgregarAvance").bind('click',function(e){
        e.preventDefault();
          ...
          $('#formulario').bind('submit', function(e) {
                ...
                $.ajax({
                  url: url,
                  data: fd+'&Actividad='+actividad+'&Estatus=on',
                  type: 'POST',
                  beforeSend: function(){
          ...
          async:true,
              ...
              success: function(response) {
                ...
                if (response[0].fields.Estatus) {
                  status='<img src="http://10.17.1.200:8000/static/base/img/check-mark-3-24.ico">';
                  console.log(status);
                }else{
                  status='<img src="http://10.17.1.200:8000/static/base/img/x-mark-24.ico">';
                  console.log(status);
                }
                $("#"+_id_Proyecto).html('<td>'+response[0].fields.Proyecto+'</td><td>'+response[0].fields.Usuario+'</td><td>'+response[0].fields.Fecha+'</td><td>'+status+'</td><td><a href="/Actividades/'+response[0].pk+'" title="Mostrar Actividades" ><img src="static/base/img/fullscreen-8-32.ico" /></a></td><td><a href="#" onclick="javascript:ajax.EditarPro('+response[0].pk+');" class="btn"><img src="static/base/img/edit-9-24.ico" /></a></td><td><a href="#" onclick="javascript:ajax.EliminarPro('+_id_Proyecto+');" class="btn btn-danger"><img src="static/base/img/trash-can-16-24.ico" /></a></td>');
       ...
       $.ajax({
          async:true,
              ...
              success: function(response) {
                ...
                $("#"+_id_Actividad).html('<td>'+response[0].fields.Actividad+'</td><td>'+response[0].fields.Proyecto+'</td><td>'+response[0].fields.Desde+'</td><td>'+response[0].fields.Hasta+'</td><td>'+status+'</td><td><a href="#" onclick="javascript:ajax.ShowDetail('+response[0].pk+');"><img src="http://10.17.1.200:8000/static/base/img/book-16-32.ico"></a><div id="Detalles'+response[0].pk+'" style="display:none;"><b>Quien labora esta actividad :</b>'+response[0].fields.Usuario+'<br><b>Fecha de Solicitud: </b>'+response[0].fields.FechaSolicitud+'<br><b>Solicitud: </b>'+response[0].fields.Solicitud+'<br><b>Observacion: </b>'+response[0].fields.Observaciones+'</div></td><td><td><td><a href="/Avances/'+response[0].pk+'" title="Mostrar Actividades" ><img src="http://10.17.1.200:8000/static/base/img/fullscreen-8-32.ico" /></a></td><td><a href="#" onclick="javascript:ajax.EditarActividad('+response[0].pk+');" class="btn"><img src="http://10.17.1.200:8000/static/base/img/edit-9-24.ico" /></a></td>');
  ...
  function SendInbox(idActividad,Envio,Usuario){
    ...
    $('#mensaje').bind('submit', function(e) {
            ...
            $.ajax({
                url: url,
                data: fd+'&Usuario='+Usuario+'&Actividad='+idActividad+'&Envio='+Envio,
                type: 'POST',
               beforeSend: function(){
                     console.log(fd);
                },
        ...
        eliminar=confirm("¿Deseas Eliminar esta Actividad?");
         if (eliminar){
          var url = '/EliminarActAjax/'+_id_Actividad;
              ...
              $.ajax({
                url: url,
                data: fd+'&actividad='+_actividad+'&Usuario='+_id_Usuario+'&Estatus=on',
                type: 'POST',
                beforeSend: function(){
